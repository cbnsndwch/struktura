...
export function pad(
    ...
    const padLength = _length - str.length;
    switch (type) {
        case 'right':
            return str + _padStr.repeat(padLength);
        case 'both':
            return (
                _padStr.repeat(Math.ceil(padLength / 2)) +
                str +
                _padStr.repeat(Math.floor(padLength / 2))
            );
        default: // 'left'
            return _padStr.repeat(padLength) + str;
    }
}

...
describe('SecureExpressionEvaluatorService', () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get<SecureExpressionEvaluatorService>(
            SecureExpressionEvaluatorService
        );
    });
    ...
    describe('safeEvaluate', () => {
        ...
        it('should throw error for division by zero', () => {
            expect(() => service.safeEvaluate('10 / 0')).toThrow(
                ExpressionEvaluationError
            );
            expect(() => service.safeEvaluate('10 % 0')).toThrow(
                ExpressionEvaluationError
        ...
        it('should throw error for unknown variables', () => {
            expect(() => service.safeEvaluate('unknownVar + 5')).toThrow(
                ExpressionEvaluationError
            );
        });

        it('should throw error for unsafe operations', () => {
            expect(() => service.safeEvaluate('Math.max(1, 2)')).toThrow(
                ExpressionEvaluationError
            );
            expect(() => service.safeEvaluate('window.alert("test")')).toThrow(
                ExpressionEvaluationError
            );
            expect(() => service.safeEvaluate('obj.property')).toThrow(
                ExpressionEvaluationError
            );
        });
        ...
        it('should throw error for disallowed characters', () => {
            expect(() =>
                service.safeEvaluate('2 + 3; console.log("hack")')
            ).toThrow(ExpressionEvaluationError);
            expect(() =>
                service.safeEvaluate('eval("malicious code")')
            ).toThrow(ExpressionEvaluationError);
        });
    ...
    describe('validateExpression', () => {
        ...
        it('should detect unsafe operations', () => {
            ...
            expect(result.isValid).toBe(false);
            expect(result.errors.some(e => e.includes('CallExpression'))).toBe(
                true
            );
        });
        ...
        it('should handle syntax errors', () => {
            ...
            expect(result.isValid).toBe(false);
            expect(result.errors.some(e => e.includes('Syntax error'))).toBe(
                true
            );
        });
    ...
    describe('security tests', () => {
        ...
        it('should prevent property access', () => {
            ...
            expect(() => service.safeEvaluate('obj.dangerous', scope)).toThrow(
                ExpressionEvaluationError
            );
            expect(() =>
                service.safeEvaluate('obj["dangerous"]', scope)
            ).toThrow(ExpressionEvaluationError);
        });

        it('should prevent function calls', () => {
            expect(() => service.safeEvaluate('alert("test")')).toThrow(
                ExpressionEvaluationError
            );
            expect(() => service.safeEvaluate('console.log("test")')).toThrow(
                ExpressionEvaluationError
            );
            expect(() => service.safeEvaluate('Math.random()')).toThrow(
                ExpressionEvaluationError
            );
        });
    });
    ...
    describe('edge cases', () => {
        ...
        it('should handle complex nested expressions', () => {
            ...
            const result = service.safeEvaluate(
                '((a + b) * (c + d)) / (a + b + c)',
                scope
            );
        ...
        it('should handle boolean values correctly', () => {
            ...
            expect(service.safeEvaluate('isActive && !isDisabled', scope)).toBe(
                true
            );
            expect(service.safeEvaluate('isActive || isDisabled', scope)).toBe(
                true
